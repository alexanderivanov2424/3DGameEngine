...
EndScreen::EndScreen(std::shared_ptr<Application> application) : Screen(application){

}

void EndScreen::onSwitch(){
    QApplication::restoreOverrideCursor();
}

void EndScreen::draw(Graphics *g){
    g->setCamera(camera);
    ...
    g->clearTransform();
    FontMetrics fm = g->getFontMetrics("default", "restart", 80.0f);
    g->translate(glm::vec3(w/2 - fm.width/2,h/2 - fm.ascent/2,0));
    g->setColor(glm::vec3(255,0,0));
    g->drawText("restart", 80.0f);
}

void EndScreen::mousePressEvent(QMouseEvent *event){
    int mouseX = event->x();
    int mouseY = event->y();
    int w = application->width;
    int h = application->height;
...
void EndScreen::mouseMoveEvent(QMouseEvent *event){
